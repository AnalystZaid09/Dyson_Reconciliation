...
def main():
    ...
    if run_button:
        ...
        excel_bytes = save_summary_to_excel_bytes(summary_row, cols_order)

        st.download_button(
            label="📥 Download summary (Excel)",
            data=excel_bytes,
            file_name=f"{os.path.splitext(pdf_file.name)[0]}_summary.xlsx",
        ...
        st.download_button(
            ...
            file_name=f"{os.path.splitext(pdf_file.name)[0]}_summary.csv",
            mime="text/csv",
        )
